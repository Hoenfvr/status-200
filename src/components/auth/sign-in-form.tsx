...
const defaultValues = { username: 'admin', password: '1234' } satisfies Values;
...
export function SignInForm(): React.JSX.Element {
  ...
  const onSubmit = React.useCallback(
    async (values: Values): Promise<void> => {
      ...
      // เรียกใช้งาน signInWithPassword ของ authClient
      const { error } = await authClient.signInWithPassword({
        username: values.username,  // ใช้ 'username' แทน 'email'
      ...
      // ถ้า sign in สำเร็จ ทำการ refresh session และ redirect
      await checkSession?.();
      router.refresh();
    },
    [checkSession, router, setError]
